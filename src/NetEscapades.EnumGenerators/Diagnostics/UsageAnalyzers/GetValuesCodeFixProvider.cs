...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(GetValuesCodeFixProvider)), Shared]
public class GetValuesCodeFixProvider : CodeFixProviderBase
{
    private const string Title = "Replace with generated GetValues()";

    public sealed override ImmutableArray<string> FixableDiagnosticIds
        => ImmutableArray.Create(GetValuesAnalyzer.DiagnosticId);

    public sealed override Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        if (!context.Diagnostics.IsDefaultOrEmpty)
        {
            // Register a code action for GetValues() replacement
        ...
        CancellationToken cancellationToken)
    {
        ...
        var newInvocation = generator.InvocationExpression(
                generator.MemberAccessExpression(generator.TypeExpression(extensionTypeSymbol), "GetValues"))
