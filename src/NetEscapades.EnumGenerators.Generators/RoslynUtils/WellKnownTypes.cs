...
// Based on https://github.com/dotnet/aspnetcore/blob/main/src/Shared/RoslynUtils/WellKnownTypes.cs#L79
...
internal class WellKnownTypes
{
    ...
    /// <summary>
    /// Returns true if the type symbol for the specified well-known type exists in the compilation
    /// </summary>
    public bool ExistsInCompilation(WellKnownTypeData.WellKnownType type)
    {
        return !SymbolEqualityComparer.Default.Equals(GetOptional(type), _missingTypeSymbol);
    }
    ...
    internal sealed class MissingType { }
}